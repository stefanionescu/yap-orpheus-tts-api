...
if [ "$CLEAN_TRT" = "1" ]; then
  ...
  # INT8 SmoothQuant quantized checkpoint directory
  rm -rf "${QUANTIZED_DIR}" || true
  # INT8 SQ + KV INT8 engine output directory
  rm -rf "${TRTLLM_ENGINE_DIR}" || true
  # Legacy engine directories (for backward compatibility)
  rm -rf "$PWD/models/orpheus-trt" || true
  ...
  if [ -d "$PWD/models" ]; then
    ...
    find "$PWD/models" -type f -name "*.trt" -delete 2>/dev/null || true
    # Quantized checkpoint files
    find "$PWD/models" -type f -name "rank*.safetensors" -delete 2>/dev/null || true
    find "$PWD/models" -type f -name "*.bin" -delete 2>/dev/null || true
    find "$PWD/models" -type f -name "model_*.pt" -delete 2>/dev/null || true
    # Checkpoint and quantized directories (all variations)
    find "$PWD/models" -type d -name "*-ckpt" -exec rm -rf {} + 2>/dev/null || true
    find "$PWD/models" -type d -name "*int8*" -exec rm -rf {} + 2>/dev/null || true
    find "$PWD/models" -type d -name "*-trt*" -exec rm -rf {} + 2>/dev/null || true
    find "$PWD/models" -type d -name "*-quantized" -exec rm -rf {} + 2>/dev/null || true
  fi
